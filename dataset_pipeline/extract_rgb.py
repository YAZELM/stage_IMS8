...
Les frames sont sauvegardees en PNG et les timestamps dans un fichier texte.

Cette sortie sert ensuite aux simulateurs video-to-event  pour generer les evenements a partir des frames RGB.
...
def extract_rgb(bag_path, out_root="outputs", rgb_topic=None):
    ...
    # On conserve la structure par sequence pour que les frames, timestamps et

    # metadonnees restent faciles a retrouver.
    ...
    with open_bag(bag_path) as bag, open(timestamps_path, "w") as f_ts:
        ...
        # Le topic RGB peut etre fourni manuellement ou detecte par les utilitaires.
        ...
        # Ce CSV minimal garde l'association index, timestamp et nom de frame.
        ...
        for msg in read_topic(bag, rgb_topic):
            ...
            # Le timestamp dans le nom du fichier permet de retrouver l'ordre

            # temporel meme sans relire le fichier de timestamps.
    ...
    # Les metadonnees documentent le topic et la resolution reellement extraits.
...
def main():
    ...
    # Point d'entree simple pour extraire une seule sequence depuis le terminal.
